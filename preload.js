...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    // Pie Menu API
    pieMenuAction: (action) => ipcRenderer.send('pie-menu-action', action),
    onPieMenuData: (callback) => ipcRenderer.on('pie-menu-data', (_event, data) => callback(data)),

    // Screenshot Send Mode
    toggleScreenshotSendMode: (isActive) => ipcRenderer.send('toggle-screenshot-send-mode', isActive),
...
let isScreenshotSendModeEnabled = false;

ipcRenderer.on('set-screenshot-send-mode', (event, isActive) => {
    console.log('GeminiDesk: Screenshot-on-Send Mode set to:', isActive);
    isScreenshotSendModeEnabled = isActive;
});

// Intercept Send for Screenshot Mode
// Note: We use the same Capture Phase logic as Prompt Prefix
window.addEventListener('click', (e) => {
    if (!isScreenshotSendModeEnabled) return;

    // Ignore programmatic clicks generated by our own logic.
    // Events generated by script (element.click() or dispatchEvent) have isTrusted = false.
    // User clicks have isTrusted = true.
    ...
    if (isSendBtn) {
        console.log('GeminiDesk: Intercepting send for Screenshot-on-Send...');
        e.stopImmediatePropagation();
        e.preventDefault();

        ipcRenderer.send('perform-screenshot-and-send');
    }
}, true); // Capture phase
...
window.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Enter' && !e.shiftKey) {
        ...
        if (isInput) {
            console.log('GeminiDesk: Intercepting Enter for Screenshot-on-Send...');
            e.stopImmediatePropagation();
            e.preventDefault();
