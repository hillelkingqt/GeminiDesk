...
async function loadGemini(mode, targetWin, initialUrl, options = {}) {
    ...
    newView.webContents.on('did-navigate', (event, url) => {
            // GeminiDesk Markdown Auto-Formatting Injection
            try {
                const markdownScript = `
                (function() {
                    if (window.geminiDeskMarkdownInitialized) return;
                    window.geminiDeskMarkdownInitialized = true;

                    const waitForQuill = setInterval(() => {
                        const editorEl = document.querySelector('.ql-editor, [data-placeholder*="Ask"]');
                        if (editorEl) {
                            const quillInstance = getQuillInstance(editorEl);
                            if (quillInstance) {
                                clearInterval(waitForQuill);
                                initializeMarkdownShortcuts(editorEl, quillInstance);
                            }
                        }
                    }, 500);

                    function getQuillInstance(editorElement) {
                        if (editorElement.__quill) return editorElement.__quill;
                        if (window.Quill && typeof window.Quill.find === 'function') {
                            const quill = window.Quill.find(editorElement);
                    ...
                    function initializeMarkdownShortcuts(editorEl, quill) {
                        console.log('GeminiDesk Markdown: Initialized.');

                        quill.on('text-change', (delta, oldDelta, source) => {
                            if (source !== 'user' || !delta.ops) return;
                            ...
                            if (selection.length === 0 && lastOp && lastOp.insert === ' ') {
                                const [line, offset] = quill.getLine(selection.index);
                                if (!line) return;

                                const textBefore = line.getText(0, offset);
                                ...
                                if (trimmedText === '*' || trimmedText === '-') {
                                    ...
                                    quill.deleteText(lineStart, len + 1);
                                }
                            }

                            // Auto-formatting for inline code
                            if (selection.length === 0 && lastOp && lastOp.insert && lastOp.insert.endsWith('\`')) {
                                const [line, offset] = quill.getLine(selection.index);
                                const lineContent = line.getText(0, offset);

                                const lastBacktick = lineContent.length - 1;
                                const secondLastBacktick = lineContent.lastIndexOf('\`', lastBacktick - 1);

                                if (secondLastBacktick !== -1) {
                                    const textBetween = lineContent.substring(secondLastBacktick + 1, lastBacktick);
                                    if (textBetween.length > 0 && !textBetween.includes('\`')) {
                                        const formatStart = selection.index - (textBetween.length + 2);
                                        const currentFormat = quill.getFormat(formatStart + 1);

                                        if (!currentFormat.code) {
                                            quill.deleteText(formatStart, textBetween.length + 2, 'api');
                                            quill.insertText(formatStart, textBetween, { 'code': true }, 'api');
                                            quill.setSelection(formatStart + textBetween.length, 0, 'api');
                                            quill.insertText(quill.getSelection().index, ' ', 'api');
                                        }
                                    }
                                }
                            }
                        ...
                        editorEl.addEventListener('keydown', (e) => {
                            ...
                            if (e.key === 'Tab' && formats.list) {
                                e.preventDefault();
                                quill.format('indent', e.shiftKey ? '-1' : '+1');
                            }

                            // Enter key handling for lists and code blocks
                            if (e.key === 'Enter') {
                                const [line, ] = quill.getLine(selection.index);
                                const lineText = line.getText().trim();
                                const lineStart = quill.getIndex(line);

                                if (formats.list && lineText === '') {
                                    // Double enter exits list
                                    if (quill.getFormat(selection.index - 1).list) {
                                        e.preventDefault();
                                        quill.formatLine(selection.index, 1, 'list', false);
                                    }
                                } else if (lineText === '\`\`\`' && !formats['code-block']) {
                                    e.preventDefault();
                                    ...
                                    e.preventDefault();
                                    quill.deleteText(lineStart, 3);
                                    quill.formatLine(lineStart, 1, 'code-block', false);
                                }
                            }

                            // Backspace handling for lists
                            if (e.key === 'Backspace' && selection.index > 0) {
                                const [line, offset] = quill.getLine(selection.index);
                                if (offset === 0 && formats.list) {
                                    e.preventDefault();
                                    quill.formatLine(selection.index, 1, 'list', false);
                                }
                            }
                ...
                `;
                view.webContents.executeJavaScript(markdownScript);
                console.log('GeminiDesk: Injected markdown auto-formatting script.');
            } catch (err) {
                console.error('GeminiDesk: Failed to inject markdown script:', err);
            }
