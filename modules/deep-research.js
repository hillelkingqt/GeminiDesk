...
async function executeScheduledDeepResearch(format) {
    try {
        ...
        // Notify user
        if (sendNotification) {
            sendNotification('Deep Research Started', 'Automated deep research session has started.', 'info');
        }
        ...
        if (view && view.webContents && !view.webContents.isDestroyed()) {
            ...
            setTimeout(() => {
                ...
                // Notify user
                if (sendNotification) {
                    sendNotification('Deep Research Completed', 'Your scheduled research session has finished successfully.', 'success');
                }
            }, 60000); // Wait a minute before sending notification
        ...
        if (sendNotification) {
            sendNotification('Deep Research Failed', 'There was an error executing your scheduled research.', 'error');
        }
